...
def index(request):
    ...
    return render(request,'index.html',{'book':book,'cate':cate,'name':name,'s_cate':s_cate})
...
def book_list(request):
    ...
    return render(request,'booklist.html',{'s_cate':s_cate,'d_cate':d_cate,'cate':cate,'page':page,'s_id':s_id,'name':name})
...
def indent_ok(request):
    ...
    order_id = TOrder.objects.filter(num=order)
    for i in book_all:
        bookid = i.book.id
        amount = i.amount
    ...
    if address_all.count() == 0:
        if tel:
            ...
            TAddress.objects.create(name=man, detail_address=address, zipcode=zipcode, addr_mobile=moble, user_id=userid,
                                    less_address=man)
            order = \
            TAddress.objects.filter(name=man, detail_address=address, zipcode=zipcode, telphone=tel, user_id=userid,
                                    less_address=man)[0]
            id = order.id
            return JsonResponse({'result': 1,'id':id,'userid':userid})
    else:
